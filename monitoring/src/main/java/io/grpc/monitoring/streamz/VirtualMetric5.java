...
/**
 * A five-dimensional VirtualMetric.
 *
...
public class VirtualMetric5<F1, F2, F3, F4, F5, V>
    extends VirtualMetric<V, VirtualMetric5<F1, F2, F3, F4, F5, V>> {
  private final Receiver<DefineCellCallback<F1, F2, F3, F4, F5, V>> definingFunction;

  /**
   * The callback that the defining function uses to define each
   * cell in the metric.
   */
  public interface DefineCellCallback<F1, F2, F3, F4, F5, V> {
  ...
  VirtualMetric5(
      ...
      Receiver<DefineCellCallback<F1, F2, F3, F4, F5, V>> definingFunction) {
  ...
  void applyToEachCell(final Receiver<GenericCell<V>> callback) {
    definingFunction.accept(new DefineCellCallback<F1, F2, F3, F4, F5, V>() {
      @Override
      public void defineCell(
          F1 field1,
      F2 field2,
      ...
      F5 field5, V value) {
        callback.accept(new VirtualCell<V>(
            VirtualMetric5.this,
            new FieldTuple(field1, field2, field3, field4, field5),
            value));
      }
    });
  }
