// Generated by generate.py

package io.grpc.monitoring.streamz;

import com.google.common.collect.ImmutableList;
import com.google.protobuf.Any;

import javax.annotation.Generated;

/**
 * A ten-dimensional EventMetric.
 *
 * <p>This class may only be instantiated through a {@link MetricFactory}.
 *
 * @param <F1> The type of the first metric field.
 * @param <F2> The type of the second metric field.
 * @param <F3> The type of the third metric field.
 * @param <F4> The type of the fourth metric field.
 * @param <F5> The type of the fifth metric field.
 * @param <F6> The type of the sixth metric field.
 * @param <F7> The type of the seventh metric field.
 * @param <F8> The type of the eighth metric field.
 * @param <F9> The type of the ninth metric field.
 * @param <F10> The type of the tenth metric field.
 * @author ecurran@google.com (Eoin Curran)
 */
@Generated(value = "generate.py")
public class EventMetric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>
    extends EventMetricBase<EventMetric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>> {
  private final Metric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, Distribution> metric;

  EventMetric10(String name, Bucketer bucketer, Metadata metadata,
      Field<F1> field1,
      Field<F2> field2,
      Field<F3> field3,
      Field<F4> field4,
      Field<F5> field5,
      Field<F6> field6,
      Field<F7> field7,
      Field<F8> field8,
      Field<F9> field9,
      Field<F10> field10) {
    super(bucketer);
    metric = new Metric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10, Distribution>(
        name, ValueTypeTraits.getTraits(Distribution.class), metadata.setCumulative(),
        ImmutableList.of(
            field1,
            field2,
            field3,
            field4,
            field5,
            field6,
            field7,
            field8,
            field9,
            field10));
    metric.setDefaultValue(new Distribution(bucketer));
  }

  /**
   * Records the given value in the distribution.
   */
  public void record(
      Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> key,
      double value) {
    if (Distribution.isValidSample(value, 1)) {
      metric.changeUnderLock(key, createChanger(value));
    }
  }

  /**
   * @see #record(Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>, double)
   */
  public void record(
      F1 field1,
      F2 field2,
      F3 field3,
      F4 field4,
      F5 field5,
      F6 field6,
      F7 field7,
      F8 field8,
      F9 field9,
      F10 field10,
      double value) {
    record(Metric10.createCellKey(
        field1, field2, field3, field4, field5, field6, field7, field8, field9, field10), value);
  }

  /**
   * Records the given value in the distribution with provided
   * com.google.protobuf.Any attachments. All provided attachments must
   * use different type URL.
   */
  public void recordWithExemplar(
      Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> key,
      double value, Any... attachments) {
    if (Distribution.isValidSample(value, 1)) {
      metric.changeUnderLock(key, createChanger(value, attachments));
    }
  }

  /**
   * Records the given value in the distribution with provided
   * com.google.protobuf.Any attachments. All provided attachments must
   * use different type URL.
   */
  public void recordWithExemplar(
      F1 field1,
      F2 field2,
      F3 field3,
      F4 field4,
      F5 field5,
      F6 field6,
      F7 field7,
      F8 field8,
      F9 field9,
      F10 field10,
      double value, Any... attachments) {
    recordWithExemplar(Metric10.createCellKey(
        field1, field2, field3, field4, field5, field6, field7, field8, field9, field10),
        value, attachments);
  }

  /**
   * Records the given value in the distribution, "count" times.
   */
  public void recordMultiple(
      Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> key,
      double value, long count) {
    if (Distribution.isValidSample(value, count)) {
      metric.changeUnderLock(key, createChanger(value, count));
    }
  }

  /**
   * @see #recordMultiple(Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>, double, long)
   */
  public void recordMultiple(
      F1 field1,
      F2 field2,
      F3 field3,
      F4 field4,
      F5 field5,
      F6 field6,
      F7 field7,
      F8 field8,
      F9 field9,
      F10 field10,
      double value, long count) {
    recordMultiple(Metric10.createCellKey(
        field1, field2, field3, field4, field5, field6, field7, field8, field9, field10),
        value, count);
  }

  /**
   * Ensures a distribution is defined for the supplied fields. If there is
   * none, one is created with zero elements. If one already exists for the
   * supplied fields, there is no change.
   *
   * @return this object.
   */
  public EventMetric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> forceCreate(
      Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> key) {
    metric.getOrCreateCell(key);
    return this;
  }

  /**
   * @see #forceCreate(Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>)
   */
  public EventMetric10<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> forceCreate(
      F1 field1,
      F2 field2,
      F3 field3,
      F4 field4,
      F5 field5,
      F6 field6,
      F7 field7,
      F8 field8,
      F9 field9,
      F10 field10) {
    return forceCreate(Metric10.createCellKey(
        field1, field2, field3, field4, field5, field6, field7, field8, field9, field10));
  }

  /**
   * Returns a copy of the distribution for the specified cell.
   */
  public Distribution get(
      Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10> key) {
    return metric.get(key);
  }

  /**
   * @see #get(Metric10.CellKey<F1, F2, F3, F4, F5, F6, F7, F8, F9, F10>)
   */
  public Distribution get(
      F1 field1,
      F2 field2,
      F3 field3,
      F4 field4,
      F5 field5,
      F6 field6,
      F7 field7,
      F8 field8,
      F9 field9,
      F10 field10) {
    return get(Metric10.createCellKey(
        field1, field2, field3, field4, field5, field6, field7, field8, field9, field10));
  }

  @Override
  protected StoredMetric<Distribution, ?> getUnderlyingMetric() {
    return metric;
  }
}